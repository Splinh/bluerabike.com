...
> Get more valuable insights with the Independent Analytics Pro plugin, which includes UTM campaign tracking, real-time analytics, click tracking, eCommerce sales tracking (WooCommerce, FluentCart, EDD, SureCart), form submission tracking, HTML email reports, and a customizable Overview report.
...
- **Feature:** Added option to group campaigns by their landing page, source, medium, or campaign name.
...
- **Enhancement:** Massively improved the WooCommerce Order Referrer metabox (renamed to Customer Journey). It now includes the customer’s time of arrival, geolocation, device data, number of pages viewed, and time of purchase.
- **Enhancement:** All date ranges from the date picker are now available for the Overview modules.
- **Enhancement:** Added support for the Curcy multi-currency plugin for WooCommerce.
- **Enhancement:** Added support for the YITH Multi Currency plugin for WooCommerce.
- **Enhancement:** Added support for WooPayments multi-currency.
...
- **Fix:** Detection added for keeping spam clicks out of the Clicks report.
- **Fix:** Stopped auto-loading options for the Overview report to reduce overhead.
- **Fix:** Filter conditions sometimes showed the wrong form name.
- **Fix:** Fixed the layout of Recent Views and Recent Conversions modules on mobile devices when given a two-column width.
- **Fix:** Email report could trigger an error due to the line length being too long
...
- **Enhancement:** Overview modules now support dates ranges of 3 months, 6 months, and 12 months
- **Enhancement:** Removed the progress bar from the Real-time analytics report
...
- **Fix:** Landing pages and Exit pages with 0 entrances/exits no longer shown in email report
- **Fix:** Overview report wasn’t getting reset when analytics are reset
...
* **Fix:** Fixed form tracking for Thrive Leads
* **Fix:** Added missing validation for the ID link pattern
...
* **Fix:** Fixed an issue where double opt-in forms were not tracked in Fluent Forms Pro
* **Fix:** Allow click tracking to avoid using the temporary directory
...
* **Fix:** Busiest Time of Day wasn’t always using correct timezone
* **Fix:** Fixed layout issue for Overview modules for RTL language sites
* **Fix:** Hebrew characters weren’t displaying in Link Target URLs
...
* **Feature:** added a new link pattern to track clicks via ID
* **Feature:** added a new API function called iawp_top_posts() for getting the most popular posts
* **Enhancement:** resetting the analytics no longer deletes the campaigns and link patterns
* **Enhancement:** Updated all translations
...
* **Fix:** resolved edge case where the Overview modules could have their data repeatedly requested causing high CPU usage
* **Fix:** line charts were not resizing properly in the Overview report
* **Fix:** fixed PHP warning in the Overview report
* **Fix:** email report delivery could be off by an hour due to DST transition
* **Fix:** the module edit buttons had an ugly white background in dark mode
* **Fix:** pie charts resize properly when switching from two- to one-column wide
* **Fix:** layout issues fixed in Recent Views & Recent Conversions modules

= 2.11.0 - April 21st, 2025 =

* **Feature:** added the new Overview report to Independent Analytics Pro!
* **Feature:** added form tracking for Divi and MailPoet
* **Update:** the email report now applies BCC to all emails after the first
...
* **Feature:** added form submission tracking for WP Store Locator
* **Update:** added "copy to clipboard" button for classes in the Click Tracking menu
* **Update:** updated translations for German, Spanish, Italian, French, and Portuguese
* **Update:** added CIDR support for IP address blocking
* **Fix:** resolved incompatibility with click tracking and the Security Optimizer plugin
* **Fix:** fixed 500 error when recording clicks on WP Engine websites
