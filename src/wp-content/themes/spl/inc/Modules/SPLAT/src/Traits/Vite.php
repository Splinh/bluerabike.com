 ...
 *
 * Based on HD\Core\Asset with improvements:
 ...
 * - Transient caching for production performance
 *
 * @package SPLAT\Traits
 */

namespace SPLAT\Traits;
...
trait Vite {
	...
	public static function manifest(): array {
		$cacheKey = 'manifest:splat';

		if ( isset( self::$manifestCache[ $cacheKey ] ) ) {
			return self::$manifestCache[ $cacheKey ];
		}

		$manifestPath = rtrim( SPLAT_PATH, '/\\' ) . '/assets/.vite/manifest.json';
	 ...
	 */
	public static function manifestResolve( ?string $entry = null, string $handlePrefix = 'splat-' ): array {
		if ( ! $entry || ! trim( $entry ) ) {
			return [];
		}
	...
	private static function assetUrl( string $file ): string {
		return SPLAT_URL . 'assets/' . ltrim( $file, '/' );
	}
